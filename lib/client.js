...
`;
		...
		*/
		...
		function SavedList(props) {
			...
			return react.createElement("div", { className: "ct-list" }, ...list.map((entry) => react.createElement("div", {
				...
				onClick: (e) => e.stopPropagation()
			}, activeId === entry.id ? react.createElement("span", { className: "ct-list-badge" }, t("custom.inUse")) : null, react.createElement(IconButton, {
		...
		*/
		...
		const zh = {
			...
			"custom.delete": "删除",
			"custom.cancel": "取消",
			"custom.toast.reset": "已重置为预设配色",
			"custom.toast.saved": "已保存",
			"custom.toast.duplicated": "已复制为新主题",
		...
		const en = {
			...
			"custom.toast.reset": "Reset to the preset colors",
			"custom.toast.saved": "Saved",
			"custom.toast.duplicated": "Duplicated as a new theme",
			...
			*/
			...
			const saved = {
				...
				async save() {
					...
					const current = active ? roster.find((e) => e.id === active) : void 0;
					if (current) await pushThemes([{
						...current,
						base: draft.base,
						theme: draft.theme
					}], fallbackForBase);
