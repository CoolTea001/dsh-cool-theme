...
`;
		...
		*/
		function ThemeCard(props) {
			...
			const action = (label, onClick, danger) => react.createElement("button", {
				type: "button",
			...
			return react.createElement("div", {
			...
			}, entry.name), active ? react.createElement("span", {
				...
				onClick: (e) => e.stopPropagation()
			}, action(t("custom.edit"), onEdit), action(t("custom.duplicate"), onDuplicate), action(t("custom.delete"), onDelete, true)));
		}
		/**
		...
		*/
		...
		function ThemePanel(props) {
			...
			async function saveEdit() {
				...
				const id = editing.id;
				if (await mutate(() => id === null ? saved.create(name) : saved.update(id, name)) === null) return;
				setEditing(null);
				syncSaved();
			...
			*/
			...
			const children = [
				...
				react.createElement("div", {
					className: "ct-row",
					key: "presets"
				}, react.createElement("div", { className: "ct-row-main" }, react.createElement("div", { className: "ct-row-title" }, t("presets.title")), react.createElement("div", { className: "ct-row-desc" }, t(customOn ? "presets.base" : "presets.desc"))), react.createElement(SchemeMenu, {
			...
			if (customOn) {
				...
				const dotRow = (key, label, dots) => react.createElement("div", {
					...
					key
				}, react.createElement("div", { className: "ct-seed-label" }, label), react.createElement("div", { className: "ct-seed-dots" }, ...dots.map((d) => react.createElement("label", {
					key: d.key,
					...
					onChange: (e) => d.onChange(e.target.value.toUpperCase())
				})))));
				...
				const neutral = {
					...
					onChange: (v) => editCustom(seedMode === "light" ? {
						...custom,
						neutralLightest: v
					} : {
						...custom,
				...
				const editorCard = (key) => react.createElement("div", {
					className: "ct-editor",
					key
				}, react.createElement("div", { className: "ct-editor-head" }, editorName, modeToggle), dotRow("seeds", t("custom.group.seeds"), seedDots), dotRow("shiki", t("custom.group.shiki"), shikiDots), editorActions);
				...
				children.push(react.createElement("div", {
					...
					disabled: editing !== null,
					onClick: startAdd
				}, react.createElement(IconPlus, null), t("custom.add")));
			}
		...
		*/
		...
		const zh = {
			...
			"presets.base": "自定义主题开启时，这里选择新主题继承的预设配色",
			"scheme.light": "浅色",
			"scheme.dark": "深色",
			"scheme.system": "跟随系统",
			"custom.title": "自定义主题",
		...
		const en = {
			...
			"presets.base": "While custom theme is on, pick the preset new themes inherit",
			"scheme.light": "Light",
			"scheme.dark": "Dark",
