...
`;
		...
		*/
		...
		/** Repair a partially-invalid object against `fallback`, keeping every valid colour. */ function normalizeCustom(raw, fallback) {
			const src = raw ?? {};
		...
		*/
		function ColorCell(props) {
			...
			return react.createElement("div", { className: "ct-cell" }, react.createElement("input", {
		...
		/** One `<tr>` of the editor table: name, then a light and a dark swatch cell. */ function SeedRow(props) {
			...
			return react.createElement("tr", null, react.createElement("td", {
				className: "ct-td-name",
				title: hint
			}, name), react.createElement("td", { className: "ct-td-color" }, react.createElement(ColorCell, {
			...
			})), react.createElement("td", { className: "ct-td-color" }, react.createElement(ColorCell, {
		...
		*/
		...
		const ICON_RENAME = [
			...
			{
				tag: "path",
				attrs: { d: "M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" }
			}
		];
		const ICON_COPY = [{
			tag: "rect",
		...
		const ICON_TRASH = [
			...
			{
				tag: "path",
				attrs: { d: "M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" }
			}
		];
		function Icon(props) {
			return react.createElement("svg", {
				...
				"aria-hidden": true,
				focusable: false
			}, ...props.nodes.map((n, i) => react.createElement(n.tag, {
				key: i,
				...n.attrs
			})));
		}
		...
		*/
		...
		function SavedList(props) {
			...
			return react.createElement("div", { className: "ct-list" }, ...list.map((entry) => react.createElement("div", {
				className: "ct-list-row",
				key: entry.id
			}, editingId === entry.id ? react.createElement("input", {
			...
			}, entry.name), react.createElement("div", { className: "ct-list-actions" }, activeId === entry.id ? react.createElement("span", { className: "ct-list-badge" }, t("custom.inUse")) : null, react.createElement(IconButton, {
			...
			*/
			...
			const children = [
				...
				react.createElement("div", {
					className: "ct-row",
					key: "presets"
				}, react.createElement("div", { className: "ct-row-main" }, react.createElement("div", { className: "ct-row-title" }, t("presets.title")), react.createElement("div", { className: "ct-row-desc" }, t("presets.desc"))), react.createElement(SchemeMenu, {
			...
			if (customOn) {
				...
				const seedRow = (key, title, desc) => react.createElement(SeedRow, {
					key,
					name: t(title),
					hint: t(desc),
				...
				*  colours, so its column labels differ from every other row. */
				const neutralRow = react.createElement(SeedRow, {
					...
					pair: {
						light: custom.neutralLightest,
						dark: custom.neutralDarkest
					},
					onChange: (p) => editCustom({
						...custom,
						neutralLightest: p.light,
						neutralDarkest: p.dark
					})
				});
				const seedRows = [
				...
				const shikiRows = SHIKI_KEYS.map((k) => react.createElement(SeedRow, {
					key: `shiki-${k}`,
					name: t(`shiki.${k}`),
				...
				const group = (key, firstColumn, rows) => react.createElement("div", {
					className: "ct-group",
					key
				}, react.createElement("div", { className: "ct-table-wrap" }, react.createElement("table", { className: "ct-table" }, react.createElement("thead", null, react.createElement("tr", null, react.createElement("th", { className: "ct-td-name" }, t(firstColumn)), react.createElement("th", { className: "ct-td-color" }, t("custom.seed.light")), react.createElement("th", { className: "ct-td-color" }, t("custom.seed.dark")))), react.createElement("tbody", null, ...rows))));
				children.push(group("seeds", "custom.table.seeds", seedRows), group("shiki", "custom.table.shiki", shikiRows), react.createElement("div", {
					...
					className: "ct-btn",
					onClick: onReset
				}, t("custom.reset")), react.createElement("button", {
					type: "button",
					className: "ct-btn ct-btn-primary",
					onClick: onSave
				}, t("custom.save"))));
				if (list.length > 0) children.push(react.createElement(SavedList, {
					key: "saved-list",
					list,
					activeId,
					t,
					onRename: (id, name) => setList(saved.rename(id, name)),
					onDuplicate,
		...
		*/
		...
		const zh = {
			...
			"custom.table.seeds": "主色/辅助色",
			"custom.table.shiki": "代码块",
		...
		const en = {
			...
			"custom.table.seeds": "Primary/supporting",
			"custom.table.shiki": "Code block",
		...
		function isValidSelection(v) {
			return !!v && (v === CUSTOM_SELECTION || NOOP.has(v) || v in PRESETS);
		}
		...
		function registerTheme(ctx) {
			...
			function loadCustom() {
				...
				return {
					base,
					theme: normalizeCustom(parseEnvelope(readCustomRaw())?.theme, fallback)
				};
			}
			...
			*/
			function rebaseCustom(nextBase) {
			...
			const writeList = (list) => writeRaw(CUSTOM_LIST_STORAGE_KEY, encodeList(list));
			...
			const saved = {
				...
				save() {
					...
					if (active) {
						const next = list.map((e) => e.id === active ? {
							...e,
							base: draft.base,
							theme: draft.theme
						} : e);
				...
				rename(id, name) {
					const next = readList().map((e) => e.id === id ? {
						...e,
						name
					} : e);
					writeList(next);
