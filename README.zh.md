...
自定义主题默认关闭。开启后会显示已保存的主题卡片和「添加自定义主题」按钮：点击该按钮会在按钮上方弹出可编辑卡片，包含主题名称输入框和配色行，点「保存」提交，点「取消」放弃。每张已保存的卡片都可以通过「编辑」按钮重新打开。

自定义主题开启时，预设主题下拉框仍然可用。此时它不再套用预设，而是选择新自定义主题的起始模板：有编辑卡片打开时，草稿会重新按该预设取色；没有打开时，当前颜色保持不变。每个配色分组一行，每个种子一个圆形色块；上方的浅色/深色切换决定色块正在编辑哪套外观的值。
